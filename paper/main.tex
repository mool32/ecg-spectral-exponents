...
\maketitle
% === Correspondence ===
\noindent\textbf{Correspondence:} tspiro@vaika.org\\
...
\textbf{Relationship to standard morphological features:} $\beta$ correlates with QRS duration ($\rho = 0.24$, $p < 10^{-68}$), as expected since bundle branch block both widens the QRS and steepens the spectrum. However, $\beta$ carries substantial information beyond QRS duration: for CLBBB vs.\ CRBBB classification, QRS duration alone achieves AUC $= 0.688$ while the $\beta$-vector achieves AUC $= 0.988$ ($\Delta$AUC $= +0.300$). The diagnostic effect of $\beta$ on CD vs.\ NORM retains 96\% of its magnitude after residualizing on QRS duration (Cohen's $d = 0.90$ vs.\ $0.93$ raw). This indicates that $\beta$ captures spatial conduction properties---the twelve-lead geometry of spectral slopes---not merely QRS width.
...
All data are publicly available: PTB-XL (\url{https://physionet.org/content/ptb-xl/}), Chapman-Shaoxing (\url{https://physionet.org/content/ecg-arrhythmia/}), CODE-15\% (\url{https://zenodo.org/records/4916206}). Analysis code is available as a Kaggle notebook: \url{https://www.kaggle.com/code/theodorspiro/the-heartbeat-at-the-edge-of-chaos} and on GitHub: \url{https://github.com/mool32/ecg-spectral-exponents}.
